...
local json = envs.loadfile(envs.device, "zorya-config/json.lua")()
local pretty = envs.loadfile(envs.device, "zorya-config/pretty.lua")()

function zorya.getEntries()
	local hand = component.invoke(envs.device, "open", "zorya-config/.zoryarc", "r")
...
function zorya.addEntry(name, handler, fs, ...)
	...
	hand = component.invoke(envs.device, "open", "zorya-config/.zoryarc", "w")
